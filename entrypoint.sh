...
# shellcheck disable=SC2016
envsubst '${JEMINI_PROXY_SERVER_NAME}' < /etc/nginx/conf.d/gemini-proxy.conf > /etc/nginx/conf.d/gemini-proxy.conf.tmp && mv /etc/nginx/conf.d/gemini-proxy.conf.tmp /etc/nginx/conf.d/gemini-proxy.conf
